...
import { analyzeUser, buildStudyPlan, buildAIStudyPlan } from '../engine/analysis.js';
...
// Lộ trình học theo môn (khung chuẩn)
router.get('/plan/:subjectId', (req, res) => {
  try {
    const plan = buildStudyPlan(req.user.id, Number(req.params.subjectId));
    res.json(plan);
  } catch (e) {
    console.error('[PLAN] Lỗi buildStudyPlan:', e.message);
    res.json({ steps: [], progress: 0 });
  }
});

// Lộ trình gợi ý từ AI theo môn
router.get('/ai-plan/:subjectId', async (req, res) => {
  try {
    const aiPlan = await buildAIStudyPlan(req.user.id, Number(req.params.subjectId));
    res.json({ aiPlan }); // aiPlan = null nếu không có AI
  } catch (e) {
    console.error('[AI-PLAN] Lỗi route:', e.message);
    res.json({ aiPlan: null });
  }
});
